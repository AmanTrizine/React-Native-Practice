...
const V2 = () => {
  ...
  const tryCamera = () => {
    ...
    try {
      setImageUri(
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQwcg9N1Wia3To2cOMP6eDvZjOomnjvD8Z7AA&usqp=CAU',
      );
    } catch (err) {
      console.log(err);
    }
  ...
  const checkCameraPermission = async () => {
    try {
      const camPermit = await Camera.requestCameraPermission();
      console.log('AAAAAA', camPermit);
        setShowModal(true);
    } catch (err) {
      console.log(err);
    }
  ...
  const onCameraInitialized = () => {
    console.log('Camera is initialized and ready to use.');
    // You can perform actions here that require the camera to be initialized
  };
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={styles.container}>
        <Image source={{uri: `file://'${imageUri}`}} style={styles.img} />
