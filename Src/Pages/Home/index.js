...
const Home = (props) => {
  ...
  const requestPermission=async()=>{
    try{
      const result = await request(PERMISSIONS.ANDROID.CAMERA)
           ...
           setPermit(result);
           if (result === 'unavailable' ||result === 'blocked' || result === 'denied') {
            alert("Permission denied. Please enable the permission in the app settings.");
  ...
  const checkPermission = async () => {
    try {
      ...
      console.log("Calendar permission status:", result);
      setPermit(result);
    } catch (error) {
      console.log(error);
    }
  ...
  try{
    if (permit==='granted'){
     const doc=await DocumentPicker.pick({
      type:[DocumentPicker.types.video,DocumentPicker.types.images,DocumentPicker.types.pdf],
       allowMultiSelection:true
     })
     console.log(doc)
    }else{
      alert("Get Permission First")
    }
  }catch(err){
...
const tryCamera=async()=>{
  try{
    ...
    setImageUri(result1.assets[0].uri)
  }catch(err)
  {
    console.log(err)
  }
...
const fromGallary=async()=>{
  try{
    const result1 = await launchImageLibrary()
    console.log(result1.assets[0].uri)
    setImageUri(result1.assets[0].uri)
  }catch(err)
  {
  ...
  return (
    <View style={{justifyContent:"center",alignItems:"center",width:"100%",height:"100%"}}>
      ...
      <View style={{width:"70%",marginTop:30}}>
        <Button title='Camera' onPress={tryCamera}/>
      </View>
      ...
      {/* <View>
        ...
        <Button title='Increment' onPress={incCounter} ></Button>
        <Button title='Decrement' onPress={decCounter} ></Button>
      </View> */}
